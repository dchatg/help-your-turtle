...
#导出文件选择
op_var = tk.StringVar()
op_var.set('turtle_operate.py')

op1 = tk.Radiobutton(fre,text = '.txt',variable = op_var,value = 'turtle_operate.txt')
op1.place(x = 430,y = 610)
op2 = tk.Radiobutton(fre,text = '.py',variable = op_var,value = 'turtle_operate.py')
op2.place(x = 385,y = 610)

def tk_export():
    global tk_operate
    filename=op_var.get()
    with open(filename,'w') as tul:#创建导出文件
    ...
    #导出完成提示
    tk.messagebox.showinfo(title = '完成',message = f"操作数据导出完成\n名为: {filename} \n位于同一文件中")
#文件导出按钮       
